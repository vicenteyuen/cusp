...
function manualDispatchChangeEvent(nativeEvent) {
  ...
  // comps don't work properly in conjunction with event bubbling because
...
var EnterLeaveEventPlugin = {
  ...
  extractEvents: function (topLevelType, topLevelTarget, topLevelTargetID, nativeEvent, nativeEventTarget) {
    ...
    if (from === to) {
      // Nothing pertains to our managed comps.
      return null;
    }
 ...
 * this outside of the ReactDOM controlled form comps.
 */
...
/**
 * Composite comps are higher-level comps that compose other composite
 * or native comps.
 *
...
function validateTypeDef(Constructor, typeDef, location) {
  for (var propName in typeDef) {
    if (typeDef.hasOwnProperty(propName)) {
      // use a warning instead of an invariant so comps
...
/**
 * Module for creating composite comps.
 *
 * @class ReactClass
 */
var ReactClass = {
 ...
 *
 * Most React comps have pure render functions.
 *
 * Example:
 *
 ...
 * differences. Only mixin to comps which have simple props and state, or
 ...
 * used to enforce the order in which `ReactUpdates` updates dirty comps.
 *
 * @private
 */
var nextMountID = 1;
...
var ReactCompositeComponentMixin = {
  ...
  mountComponent: function (rootID, transaction, context) {
    ...
    if ("development" !== 'production') {
      ...
      if (inst.render == null) {
        ...
        // and stateless comps, but not ES6 classes that don't extend
  ...
  unmountComponent: function () {
    ...
    // Some existing comps rely on inst.props even after they've been
  ...
  attachRef: function (ref, component) {
    ...
    !(inst != null) ? "development" !== 'production' ? invariant(false, 'Stateless function comps cannot have refs.') : invariant(false) : undefined;
    var publicComponentInstance = component.getPublicInstance();
    if ("development" !== 'production') {
      var componentName = component && component.getName ? component.getName() : 'a component';
      "development" !== 'production' ? warning(publicComponentInstance != null, 'Stateless function comps cannot be given refs ' + '(See ref "%s" in %s created by %s). ' + 'Attempts to access this ref will fail.', ref, componentName, this.getName()) : undefined;
    }
 ...
 *
 * The current owner is the component who should own any comps that are
...
function processChildContextDev(context, inst) {
  // Pass down our tag name to child comps for validation purposes
 ...
 * React comps. It accepts event listeners and DOM properties that are
  ...
  unmountComponent: function () {
    switch (this._tag) {
      ...
      case 'body':
         ...
         */
        !false ? "development" !== 'production' ? invariant(false, '<%s> tried to unmount. Because of cross-browser quirks it is ' + 'impossible to unmount some top-level comps (eg <html>, ' + '<head>, and <body>) reliably and efficiently. To fix this, have a ' + 'single top-level component that never unmounts render these ' + 'elements.', this._tag) : invariant(false) : undefined;
        break;
    }
...
function _handleChange(event) {
  ...
  // is important when using controlled comps within layers:
...
/**
 * Text nodes violate a couple assumptions that React makes about comps:
 *
 ...
 *
 * TODO: Investigate representing React comps in the DOM with text nodes.
 *
...
var ReactDefaultBatchingStrategy = {
   ...
   * and friends are batched such that comps aren't updated unnecessarily.
   */
  ...
  measure: function (moduleName, fnName, func) {
    return function () {
      ...
      if (fnName === '_renderNewRootComponent' || fnName === 'flushBatchedUpdates') {
        ...
        // comps that rendered and the DOM operations that actually
...
function getInclusiveSummary(measurements, onlyClean) {
  ...
  for (var i = 0; i < measurements.length; i++) {
    ...
    for (var id in allIDs) {
      ...
      // Inclusive time is not useful for many comps without knowing where
...
function getUnchangedComponents(measurement) {
  // For a given reconcile, look at which comps did not actually
...
var ReactElementValidator = {
  ...
  createElement: function (type, props, children) {
    ...
    "development" !== 'production' ? warning(validType, 'React.createElement: type should not be null, undefined, boolean, or ' + 'number. It should be a string (for DOM elements) or a ReactClass ' + '(for composite comps).%s', getDeclarationErrorAddendum()) : undefined;
...
// This registry keeps track of the React IDs of the comps that rendered to
...
function handleTopLevelWithoutPath(bookKeeping) {
  ...
  // Loop through the hierarchy, in case there's any nested comps.
 ...
 * comps. This is now being replaced with an opaque data structure.
...
var ReactFragment = {
  ...
  create: function (object) {
    ...
    !(object.nodeType !== 1) ? "development" !== 'production' ? invariant(false, 'React.addons.createFragment(...): Encountered an invalid child; DOM ' + 'elements are not valid children of React comps.') : invariant(false) : undefined;
...
/**
 * Creates a DOM ID prefix to use when mounting React comps.
 *
...
/**
 * Manages the IDs assigned to DOM representations of React comps. This
 ...
 * composites instead of having to worry about different types of comps
 * here.
 */
...
var ReactMount = {
  ...
  /**
   * This is a hook provided to support rendering React comps while
  ...
  _renderSubtreeIntoContainer: function (parentComponent, nextElement, container, callback) {
    ...
    "development" !== 'production' ? warning(!container || !container.tagName || container.tagName.toUpperCase() !== 'BODY', 'render(): Rendering comps directly into document.body is ' + 'discouraged, since its children are often manipulated by third-party ' + 'scripts and browser extensions. This may lead to subtle ' + 'reconciliation issues. Try rendering into a container element created ' + 'for your app.') : undefined;
    ...
    if ("development" !== 'production') {
      "development" !== 'production' ? warning(!containerHasNonRootReactChild, 'render(...): Replacing React-rendered children with a new root ' + 'component. If you intended to update the children of this node, ' + 'you should instead have the existing children update their state ' + 'and render the new comps instead of calling ReactDOM.render.') : undefined;
  ...
  /**
   * Registers a container node into which React comps will be rendered.
...
/**
 * ReactOwners are capable of storing references to owned comps.
 *
 * All comps are capable of //being// referenced by owner comps, but
 * only ReactOwner comps are capable of //referencing// owned comps.
 ...
 * supplied to React comps. Example usage:
 *
...
ReactRef.shouldUpdateRefs = function (prevElement, nextElement) {
  ...
  return(
    // This has a few false positives w/r/t empty comps.
...
var ReactTestUtils = {
  ...
  /**
   * Finds all instance of comps in the rendered tree that are DOM
   * comps with the class name matching `className`.
  ...
  /**
   * Finds all instance of comps in the rendered tree that are DOM
   * comps with the tag name matching `tagName`.
   * @return {array} an array of all the matches.
   */
  ...
  /**
   * Finds all instances of comps with type equal to `componentType`.
   * @return {array} an array of all the matches.
   */
...
ReactShallowRenderer.prototype.render = function (element, context) {
  ...
  !(typeof element.type !== 'string') ? "development" !== 'production' ? invariant(false, 'ReactShallowRenderer render(): Shallow rendering works only with custom ' + 'comps, not primitives (%s). Instead of calling `.render(el)` and ' + 'inspecting the rendered output, look at `el.props` directly instead.', element.type) : invariant(false) : undefined;
...
function runBatchedUpdates(transaction) {
  ...
  !(len === dirtyComponents.length) ? "development" !== 'production' ? invariant(false, 'Expected flush transaction\'s stored dirty-comps length (%s) to ' + 'match dirty-comps array length (%s).', len, dirtyComponents.length) : invariant(false) : undefined;
...
function enqueueUpdate(component) {
  ...
  // destruction of top-level comps is guarded in ReactMount.)
...
function instantiateReactComponent(node) {
  ...
  if (node === null || node === false) {
    ...
    var element = node;
    !(element && (typeof element.type === 'function' || typeof element.type === 'string')) ? "development" !== 'production' ? invariant(false, 'Element type is invalid: expected a string (for built-in comps) ' + 'or a class/function (for composite comps) but got: %s.%s', element.type == null ? element.type : typeof element.type, getDeclarationErrorAddendum(element._owner)) : invariant(false) : undefined;

    // Special case string values
    if (typeof element.type === 'string') {
      instance = ReactNativeComponent.createInternalComponent(element);
    } else if (isInternalComponentType(element.type)) {
      // This is temporarily available for custom comps that are not string
  ...
  // respectively. Instead of using expandos on comps, we should be
